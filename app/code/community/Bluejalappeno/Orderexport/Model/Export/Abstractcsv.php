...
abstract class Bluejalappeno_Orderexport_Model_Export_Abstractcsv extends Mage_Core_Model_Abstract
{
    ...
    {
        // $cca = $order->getPayment()->getData('authorize_cards');
        // 		$ccb = $cca['cc_last4'];
        // 		return $ccb;
		return $order->getPayment()->getData('authorize_cards')->getXType();
    }
    ...
    protected function getCcType($order)
    {
		...
		$transactionId = $additional[$keys[0]]['cc_type'];
		return $transactionId;
    }

    protected function getCcLast4($order)
    {	
		$orderData = $order->getPayment()->getData();
		$additional = $orderData['additional_information']['authorize_cards'];
		$keys = array_keys($additional);
		$transactionId = $additional[$keys[0]]['cc_last4'];
		return $transactionId;
	}
	...
	protected function getBaseGrand($order)
    {	
		$base = $order->getData('base_discount_amount');  // 26
		$x = $order->getData('base_subtotal'); // 104
		$y = $x + $base; // 76
    ...
    protected function getDiscountPercent($item)
    {
	...
	// FUELED: Outputs Shopping Cart Rule ID
	protected function getAppliedRuleIds($item){
	 return $item->getAppliedRuleIds();
	}
	
	// FUELED: Gift Code
    protected function getGiftCode($order)
    {	
		$code = $order->getGiftCodes();
		$amount = $order->getGiftVoucherDiscount();
		return $code;
	}
	
	// FUELED: Gift Code
    protected function getGiftDiscount($order)
    {	
		$code = $order->getGiftCodes();
		$amount = $order->getGiftVoucherDiscount();
		return $amount;

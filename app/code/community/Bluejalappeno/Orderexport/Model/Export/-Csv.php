...
class Bluejalappeno_Orderexport_Model_Export_Csv extends Bluejalappeno_Orderexport_Model_Export_Abstractcsv
{
    ...
    protected function getCommonOrderValues($order)
    {
        ...
        return array(
			...
			Mage::helper('core')->formatDate($order->getCreatedAt(), 'small', true),
			$this->getCcType($order),
			$this->getCcLast4($order),
			...
			$billingAddress ? $this->getStreet($billingAddress) : '',
			$billingAddress ? $billingAddress->getData("postcode") : '',
			$billingAddress ? $billingAddress->getData("city") : '',
			$billingAddress ? $billingAddress->getRegionCode() : '',
			$billingAddress ? $billingAddress->getData("company") : '',
			$billingAddress ? $billingAddress->getData("telephone") : '',
        );
    }

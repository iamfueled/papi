...
abstract class Bluejalappeno_Orderexport_Model_Export_Abstractcsv extends Mage_Core_Model_Abstract
{
    ...
    {
        return $order->getPayment()->getCcLast4();
    }
	...
	// FUELED: Outputs Shopping Cart Rule ID
	// protected function getAppliedRuleIds($item){
	// 	$item = $formatter->formatAppliedRuleIdTxt($item);
	// 	$item = str_replace('7', 'YUP', $item);
	// 	return $item;
	// }
	
	// protected function formatRule($rule, $formatter){
	// 	$rule = $formatter->formatRuleIdTxt($rule);
	// 	$rule = str_replace('7', 'YUP', $item);
	// 	return $rule;
	// }
